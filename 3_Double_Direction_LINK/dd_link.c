...
void main()
{
	...
	InitDDLink(&L);
	InsertDDLink(&L,strdup("It"));
	InsertDDLink(&L,strdup("is"));
	InsertDDLink(&L,strdup("ok"));
	InsertDDLink(&L,strdup("!"));

		
	DDLinkForeach(&L, DDLinkForeachPrint,print_str);

	DDLinkForeach(&L, DDLinkVisitor_Lower2Upper,NULL);
